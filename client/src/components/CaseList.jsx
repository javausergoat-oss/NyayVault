  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Active Investigation Vaults</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Select an investigation file to access evidence, transcripts, and cryptographic audits.</p>
        </div>
        <button 
          className="bg-[#1b4d3e] hover:bg-[#143c30] text-white flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold shadow-xs transition-colors" 
          onClick={() => setShowModal(true)}
        >
          <Plus size={16} /> Open New Case File
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-16 rounded-2xl flex flex-col items-center justify-center text-center text-slate-400">
          ...
          <p className="text-sm font-bold text-slate-700 dark:text-slate-300">No active cases found.</p>
          <p className="text-xs mt-1">Open a new case file to start digitizing evidence.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {cases.map((c) => (
            <div 
              key={c.id} 
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-emerald-500 dark:hover:border-emerald-500 p-5 rounded-2xl cursor-pointer transition-all shadow-xs flex flex-col justify-between space-y-4 group"
            ...
            >
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center text-xs">
                <span className="text-slate-400 font-medium text-[11px]">
                  {c.document_count || 0} Evidence Artifacts
                </span>
                <span className="flex items-center gap-1 font-bold text-emerald-700 dark:text-emerald-400 text-xs group-hover:translate-x-1 transition-transform">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl w-full max-w-md shadow-2xl space-y-4 text-xs">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
              <h2 className="text-base font-bold text-slate-900 dark:text-white">Open New Case File</h2>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                <button type="button" className="px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-semibold" onClick={() => setShowModal(false)}>Cancel</button>
                <button type="submit" className="px-4 py-2 rounded-xl bg-[#1b4d3e] hover:bg-[#143c30] text-white font-bold shadow-xs">Create File</button>
              </div>
            </form>
          </div>
        </div>
