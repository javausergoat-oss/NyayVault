...
async function extractTextFromStream(stream, mimeType) {
  ...
  if (mimeType.startsWith('video/') || mimeType.startsWith('audio/')) {
    console.log(`[Worker] Detected Audio/Video stream. Initiating multi-modal analysis demo...`);
    // Read stream to discard it (so MinIO doesn't hang)
    for await (const chunk of stream) {}
    
    // Simulate processing time for demo realism
    await new Promise(resolve => setTimeout(resolve, 3000));
    
    return `[SYSTEM ALERT: MULTI-MODAL ANALYSIS INITIATED]
Transcription (Auto-generated by Whisper AI Mock):
"Okay, we have visual on the suspect vehicle. It's a white SUV, license plate ending in 4402. The suspect is wearing a dark jacket. They are approaching the side entrance."

Audio Analysis: Stress markers detected in speaker's voice.
Video Analysis: 1 person identified. Vehicle: White SUV. License plate partially visible: 4402.
`;
  }
